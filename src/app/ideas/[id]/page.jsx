...
//           {/* image */}
//           <div className="relative h-64 sm:h-[420px] rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800">
//             <Image
//               src={
//                 idea.imageURL ||
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-10 shadow-sm space-y-8">
          <div className="flex flex-col sm:flex-row justify-between gap-6">
            <div className="space-y-3 flex-1">
              ...
              <h1 className="text-3xl sm:text-4xl font-bold leading-tight">{idea.title}</h1>
              <p className="text-slate-500 dark:text-slate-400">{idea.shortDescription}</p>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl flex items-center gap-4">
              ...
              />
              <div>
                <p className="text-[10px] uppercase font-black text-slate-400">Creator</p>
                <p className="text-sm font-bold">{idea.userName}</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative h-64 sm:h-[420px] rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800">
            <Image
              src={idea.imageURL || "https://images.unsplash.com/photo-1519389950473-47ba0277781c?q=80&w=1200"}
              alt={idea.title}
              fill
              priority
              className="object-cover"
          ...
          <div className="space-y-8">
            <div>
              <h3 className="text-xs font-black uppercase text-cyan-500 mb-2">Detailed Description</h3>
              <p className="text-sm sm:text-base leading-relaxed text-slate-600 dark:text-slate-300">{idea.description}</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-2xl border border-slate-200 dark:border-slate-800">
                <h3 className="text-[11px] uppercase font-bold text-slate-400 mb-1">Target Audience</h3>
                <p className="font-bold">{idea.targetAudience}</p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-2xl border border-slate-200 dark:border-slate-800">
                <h3 className="text-[11px] uppercase font-bold text-slate-400 mb-1">Estimated Budget</h3>
                <p className="font-bold text-emerald-500">
            ...
            <div className="border-l-4 border-red-500 bg-red-50/30 dark:bg-red-950/10 p-5 rounded-r-2xl">
              <h3 className="text-xs font-black uppercase text-red-500 mb-2">Problem Statement</h3>
              <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">{idea.problemStatement}</p>
            </div>

            <div className="border-l-4 border-emerald-500 bg-emerald-50/30 dark:bg-emerald-950/10 p-5 rounded-r-2xl">
              <h3 className="text-xs font-black uppercase text-emerald-500 mb-2">Proposed Solution</h3>
              <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">{idea.proposedSolution}</p>
            </div>

            {idea.tags && idea.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {idea.tags.map((tag, index) => (
                  <span key={index} className="text-[11px] font-bold bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-lg text-slate-500">
                    #{tag}
                  </span>
                ))}
              </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-10 shadow-sm space-y-6">
          <h2 className="text-xl font-bold flex items-center gap-2">
            Community Comments
            <span className="text-sm bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">{comments.length}</span>
          </h2>
          ...
          <div className="space-y-4 pt-6 border-t border-slate-100 dark:border-slate-800">
            {comments.length === 0 ? (
              <p className="text-center py-10 text-slate-400 text-sm">No comments yet.</p>
            ) : (
              comments.map((comment) => (
                <div key={comment._id} className="p-5 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 flex gap-4">
                  <Image
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex justify-between gap-4">
                      ...
                      {currentUser.email?.toLowerCase() === comment.userEmail?.toLowerCase() && (
                        <div className="flex gap-3 text-[10px] font-black uppercase">
                          {editingCommentId === comment._id ? (
                            <>
                              <button onClick={() => handleEditComment(comment._id)} className="text-emerald-500 hover:text-emerald-600 cursor-pointer">Save</button>
                              <button onClick={() => { setEditingCommentId(null); setEditingText(""); }} className="text-slate-400 hover:underline cursor-pointer">Cancel</button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => { setEditingCommentId(comment._id); setEditingText(comment.commentText); }} className="text-cyan-500 hover:text-cyan-600 cursor-pointer">Edit</button>
                              <button onClick={() => handleDeleteComment(comment._id)} className="text-red-500 hover:text-red-600 cursor-pointer">Delete</button>
                            </>
                          )}
                        </div>
                      )}
                    ...
                    {editingCommentId === comment._id ? (
                      ...
                      <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed break-words">{comment.commentText}</p>
                    )}
                  </div>
                </div>
              ))
