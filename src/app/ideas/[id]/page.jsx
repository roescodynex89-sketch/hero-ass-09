...
export default function IdeaDetailsPage() {
  ...
  useEffect(() => {
    if (isSessionPending || !id || !currentUser) return;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
      <div className="max-w-4xl mx-auto space-y-8">
        <button
          ...
          className="group flex items-center gap-2 text-xs font-bold uppercase text-slate-500 hover:text-cyan-500 transition cursor-pointer outline-none"
        >
          <span className="transform group-hover:-translate-x-1 transition-transform">←</span> Back to Ecosystem Space
        </button>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-3xl p-6 sm:p-10 shadow-sm hover:shadow-md transition-shadow duration-300 space-y-8">
          <div className="flex flex-col sm:flex-row justify-between items-start gap-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200/60 dark:border-slate-800 p-4 rounded-2xl flex items-center gap-4 shrink-0 w-full sm:w-auto shadow-2xs">
              ...
              />
              <div>
                <p className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase">ARCHITECT</p>
                <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{idea.userName}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <div className="space-y-2">
              <h3 className="text-xs font-black uppercase text-cyan-500">Detailed Vision</h3>
              <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 leading-relaxed bg-slate-50/50 dark:bg-slate-950/30 p-5 rounded-2xl border border-slate-100/50 dark:border-slate-800/30">
                {idea.description}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-2xl border border-slate-200/60 dark:border-slate-800 flex flex-col justify-center">
                <h3 className="text-[11px] font-bold uppercase text-slate-400 mb-1">Target Audience</h3>
                <p className="text-sm sm:text-base font-bold text-slate-700 dark:text-slate-200">{idea.targetAudience}</p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-950 p-5 rounded-2xl border border-slate-200/60 dark:border-slate-800 flex flex-col justify-center">
                <h3 className="text-[11px] font-bold uppercase text-slate-400 mb-1">Operational Budget</h3>
                <p className="text-sm sm:text-base font-black text-emerald-500">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="border-l-4 border-red-500 bg-red-50/30 dark:bg-red-950/10 p-5 rounded-r-2xl border border-y-slate-100 border-r-slate-100 dark:border-y-slate-800/40 dark:border-r-slate-800/40">
                <h3 className="text-xs font-black uppercase tracking-wider text-red-500 mb-2">Problem Statement</h3>
                <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{idea.problemStatement}</p>
              </div>

              <div className="border-l-4 border-emerald-500 bg-emerald-50/30 dark:bg-emerald-950/10 p-5 rounded-r-2xl border border-y-slate-100 border-r-slate-100 dark:border-y-slate-800/40 dark:border-r-slate-800/40">
                <h3 className="text-xs font-black uppercase tracking-wider text-emerald-500 mb-2">Proposed Solution</h3>
                <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{idea.proposedSolution}</p>
              </div>
            </div>
            ...
            {idea.tags && idea.tags.length > 0 && (
              <div className="pt-2 flex items-center gap-2 flex-wrap">
                {idea.tags.map((tag, i) => (
                  <span key={i} className="text-[11px] font-bold bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-lg text-slate-500 dark:text-slate-400 border border-slate-200/30 dark:border-slate-700/30">
                    #{tag}
                  </span>
                ))}
              </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-10 shadow-sm space-y-6">
          ...
          <div className="space-y-4 pt-6 border-t border-slate-100 dark:border-slate-800">
            {comments.length === 0 ? (
              ...
              comments.map((comment) => (
                <div key={comment._id} className="p-5 bg-slate-50/60 dark:bg-slate-950/40 rounded-2xl border border-slate-200/50 dark:border-slate-800/60 flex gap-4 items-start transition-all duration-200">
                  <Image
                  ...
                  <div className="flex-1 space-y-1.5 min-w-0">
                    <div className="flex justify-between items-start gap-4">
                      <div className="min-w-0">
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block truncate">{comment.userName}</span>
                        <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">
                      ...
                      {currentUser && currentUser.email === comment.userEmail && (
                        <div className="flex gap-3 text-[10px] font-black uppercase tracking-wider shrink-0">
                          {editingCommentId === comment._id ? (
                            <>
                              <button onClick={() => handleEditComment(comment._id)} className="text-emerald-500 hover:text-emerald-600 cursor-pointer transition">Save</button>
                              <button onClick={() => setEditingCommentId(null)} className="text-slate-400 dark:text-slate-500 hover:underline cursor-pointer transition">Cancel</button>
                            </>
                          ) : (
                            <>
                              <button
                              ...
                              <button onClick={() => handleDeleteComment(comment._id)} className="text-red-500 hover:text-red-600 cursor-pointer transition">Delete</button>
                            </>
                          )}
                        </div>
                      )}
                    ...
                    {editingCommentId === comment._id ? (
                      ...
                      <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed wrap-break-word">{comment.commentText}</p>
                    )}
                  </div>
                </div>
              ))
