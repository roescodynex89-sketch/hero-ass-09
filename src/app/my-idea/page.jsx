...
export default function MyIdeasPage() {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      ...
      setMyIdeas((prev) =>
        prev.map((idea) =>
          idea._id === activeUpdateIdea._id ? { ...idea, ...payload } : idea
        )
      );
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        <div className="mb-10 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-3xl font-black">My Ideas</h1>
            <p className="text-sm text-slate-500 mt-1">Manage your startup ideas</p>
          </div>
          <button
            onClick={() => router.push("/add-idea")}
            className="px-4 py-2.5 bg-cyan-500 text-white text-xs font-bold rounded-xl shadow-md hover:opacity-90 transition cursor-pointer"
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                  {myIdeas.map((idea) => (
                    <tr key={idea._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20 transition">
                      <td className="p-4">
                        <p className="font-bold line-clamp-1">{idea.title}</p>
                        <p className="text-xs text-slate-400 line-clamp-1 mt-1">{idea.shortDescription}</p>
                      </td>
                      <td className="p-4">
                        <span className="text-xs font-bold px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded-md">{idea.category}</span>
                      </td>
                      <td className="p-4 font-bold text-emerald-500">
                        {idea.estimatedBudget ? `$${idea.estimatedBudget}` : "Flexible"}
                      </td>
                      <td className="p-4 text-right space-x-2">
                        <button onClick={() => openUpdateModal(idea)} className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-xs font-bold rounded-lg hover:bg-cyan-500 hover:text-white transition cursor-pointer">Edit</button>
                        <button onClick={() => setActiveDeleteId(idea._id)} className="px-3 py-1.5 bg-red-500/10 text-red-500 text-xs font-bold rounded-lg hover:bg-red-500 hover:text-white transition cursor-pointer">Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-md rounded-2xl p-6 shadow-xl">
              ...
              <form onSubmit={handleUpdateSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full mt-1 p-3 rounded-xl border dark:border-slate-700 bg-white dark:bg-slate-900 outline-none focus:ring-2 focus:ring-cyan-500"
                    >
                      <option value="Tech">Tech</option>
                      <option value="AI">AI</option>
                      <option value="Health">Health</option>
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <button type="button" onClick={() => setActiveUpdateIdea(null)} className="px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold cursor-pointer">Cancel</button>
                  <button type="submit" className="px-4 py-2 bg-cyan-500 text-white rounded-lg text-sm font-semibold cursor-pointer">Save Changes</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {activeDeleteId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-sm rounded-2xl p-6 shadow-xl text-center">
              <div className="w-14 h-14 mx-auto rounded-full bg-red-500/10 flex items-center justify-center text-2xl">⚠️</div>
              <h3 className="mt-4 text-lg font-bold">Delete Idea?</h3>
              <p className="text-sm text-slate-400 mt-2">This action cannot be undone.</p>
              <div className="flex justify-center gap-3 mt-6">
                <button onClick={() => setActiveDeleteId(null)} className="px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold cursor-pointer">Cancel</button>
                <button onClick={handleDelete} className="px-4 py-2 bg-red-500 text-white rounded-lg text-sm font-semibold cursor-pointer">Delete</button>
              </div>
            </div>
          </div>
        )}
